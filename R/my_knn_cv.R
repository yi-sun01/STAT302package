...
my_knn_cv <- function(train, cl, k_nn, k_cv) {
  ...
  for (j in 1:k_nn) {
    ...
    # intialize a vector to store the missclassification rate
    ...
    for (i in 1:k_cv) {
      ...
      result <-  class::knn(train = data_train, test = data_test, cl = cl_train, k = j)
      my_class[new_fold] <- as.character(result)

      # compute missclassification rate
      cv_error[i] <- base::mean(result != cl_test)
